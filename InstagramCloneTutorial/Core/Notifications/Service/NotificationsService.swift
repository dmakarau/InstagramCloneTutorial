...
class NotificationsService {
    ...
    func uploadNotification(toUid uid: String, type: IGNotificationType, post: Post? = nil) async throws {
        ...
        guard let currentUid = Auth.auth().currentUser,
              uid != currentUid.uid
        else { return }
        
        // create a reference to the notifications collection
        let ref = FirebaseConstants.NotificationsCollection.document(uid).collection("user-notifications").document()
        ...
        let notification = IGNotification(
            ...
            postId: post?.id,
            timestamp: Timestamp(),
            notificationSenderUid: currentUid.uid,
            type: type
        )
